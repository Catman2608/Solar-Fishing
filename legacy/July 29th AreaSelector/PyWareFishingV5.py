...
#   2. That's it — selector UI, save/load, defaults, and visibility menu
...
class AreaSelector:
    ...
    Python is only needed for:
      - supplying area config + initial data  (get_area_config / get_areas)
      - receiving live mouse status  (on_mouse_move)
      - receiving final saved areas  (save_areas)
      - optional visibility updates   (set_visibility)
    ...
    Coordinate space notes (DPI / Retina fix):
      - Areas are stored as ratios (0-1) relative to the primary monitor.
      - JS works exclusively in CSS logical pixels (window.innerWidth/Height).
      - On Windows with DPI scaling (e.g. 125%), mss/pywebview use physical
        pixels while the embedded browser uses logical CSS pixels
        (physical / devicePixelRatio). Using the logical size reported by JS
        via window_ready() keeps boxes on-screen on both Windows and macOS.
      - macOS menu-bar offset is applied as a ratio so it stays consistent
        regardless of the logical canvas size.
    """
    def __init__(self, parent, areas_dict, callback):
        """
        areas_dict: {name: {x,y,width,height} ratios} for known AREA_ORDER keys.
        callback:   callable(areas_dict) fired on save/close.
        """
        ...
        # Logical CSS canvas size reported by JS (window.innerWidth/Height).
        self._canvas_w = float(SCREEN_WIDTH) if SCREEN_WIDTH else 1.0
        self._canvas_h = float(SCREEN_HEIGHT) if SCREEN_HEIGHT else 1.0
        self._dpr = 1.0
        # Store areas as RATIOS (0-1 range). Fall back to AREA_CONFIG defaults.
        self._areas = {}
        ...
        # Visibility (session only; all start visible)
        ...
        if sys.platform == "win32":
            self._win = webview.create_window(
                "Area Selector", self.area_selector_file, js_api=self,
    ...
    def _to_ratios(self, area):
        ...
        return {
            ...
            "width": float(area.get("width", area.get("w", 0)) or 0),
            "height": float(area.get("height", area.get("h", 0)) or 0),
        }

    def _canvas_size(self):
        """Return (width, height) of the logical CSS canvas used by JS."""
        w = self._canvas_w if self._canvas_w > 1 else float(SCREEN_WIDTH or 1)
        h = self._canvas_h if self._canvas_h > 1 else float(SCREEN_HEIGHT or 1)
        return max(1.0, w), max(1.0, h)

    def _menu_offset_ratio(self):
        """macOS menu-bar height as a fraction of the logical canvas height."""
        menu_offset = get_macos_menu_offset()
        _, h = self._canvas_size()
        return (menu_offset / h) if h > 0 else 0.0

    def _ratios_to_pixels(self, ratios, add_offset=True):
        """Convert ratio dict to logical CSS-pixel coordinates relative to canvas."""
        cw, ch = self._canvas_size()
        x_pixels = ratios["x"] * cw
        y_pixels = ratios["y"] * ch
        w_pixels = ratios["width"] * cw
        h_pixels = ratios["height"] * ch
        if add_offset:
    ...
    def _pixels_to_ratios(self, pixels, subtract_offset=True):
        """Convert logical CSS-pixel dict to ratios (0-1 range)."""
        cw, ch = self._canvas_size()
        x_pixels = float(pixels.get("x", 0) or 0)
        y_pixels = float(pixels.get("y", 0) or 0)
        if subtract_offset:
            x_pixels -= self._win_origin_x
            y_pixels -= self._win_origin_y
        return {
            "x": x_pixels / cw,
            "y": y_pixels / ch,
            "width": float(pixels.get("w", pixels.get("width", 0)) or 0) / cw,
            "height": float(pixels.get("h", pixels.get("height", 0)) or 0) / ch,
        }
    ...
    def set_visibility(self, visible_dict):
        """Optional: keep Python in sync when the panel toggles boxes."""
    ...
    def window_ready(self, win_x, win_y, logical_w=None, logical_h=None, dpr=None):
        """
        Called by JS immediately after pywebviewready fires.
        Passes window.screenX/Y plus the browser's logical CSS size
        ...
        if sys.platform == "darwin":
            ...
            self._win_origin_x = int(win_x) if win_x is not None else 0
            self._win_origin_y = int(win_y) if win_y is not None else 0

        if logical_w is not None and float(logical_w) > 1:
            self._canvas_w = float(logical_w)
        if logical_h is not None and float(logical_h) > 1:
            self._canvas_h = float(logical_h)
        if dpr is not None:
            try:
    ...
    def get_areas(self):
        ...
        Returns logical CSS-pixel coordinates relative to the canvas.
        macOS menu-bar offset is subtracted so boxes align with the desktop
        content under the pushed-down frameless window.
        """
        out = {}
        menu_offset = get_macos_menu_offset()
    ...
    def on_mouse_move(self, mouse_x, mouse_y, current_boxes):
        ...
        for name in AREA_ORDER:
            ...
            if b:
                adjusted = dict(b)
                adjusted["y"] = float(b.get("y", 0) or 0) + menu_offset
                self._areas[name] = self._pixels_to_ratios(adjusted, subtract_offset=False)

        # Hit-test for status display
        cw, ch = self._canvas_size()
        abs_x = float(mouse_x or 0) + self._win_origin_x
        abs_y = float(mouse_y or 0) + self._win_origin_y
        for name in AREA_ORDER:
            ...
            bx = ratios["x"] * cw
            by = ratios["y"] * ch
            bw = (ratios["width"] * cw) or 1.0
            bh = (ratios["height"] * ch) or 1.0
            if bx <= abs_x <= bx + bw and by <= abs_y <= by + bh:
                xr = round((abs_x - bx) / bw, 2)
                yr = round((abs_y - by) / bh, 2)
                self.parent.set_status(f"{name.upper()} → X: {xr:.2f}  Y: {yr:.2f}")
                return

    def save_areas(self, areas):
        """
        Called by JS when the user presses Escape/F6 (or Done) to confirm and close.
        JS sends canvas-relative logical CSS {x,y,width,height} pixels; we convert
        to ratios and fire the callback with a full dict.
        """
        if not self._open:
            return
        ...
        for name in AREA_ORDER:
            ...
            if b:
                ...
                # Keep previous value if JS omitted a box
                out[name] = self._areas.get(name, AREA_CONFIG[name]["default"])
        self.callback(out)
    ...
    def _on_closed(self):
        ...
        if self._open:
            self._open = False
            self.callback(dict(self._areas))
            self.parent.set_status("Area selector closed")
...
class Api:
    ...
    def open_area_selector(self):
        ...
        # Build current areas dict (saved values or AREA_CONFIG defaults)
        areas = {}
        for name in AREA_ORDER:
            a = self.bar_areas.get(name)
            areas[name] = a if isinstance(a, dict) else dict(AREA_CONFIG[name]["default"])

        # Callback receives a full {name: ratios} dict
        def on_done(new_areas):
            if isinstance(new_areas, dict):
                for name, val in new_areas.items():
                    if name in AREA_ORDER and isinstance(val, dict):
                        self.bar_areas[name] = val
            self.save_misc_settings()
            self.area_selector = None

        # Open the pywebview overlay — data-driven via AREA_CONFIG
    ...
    def _enter_minigame(self):
        ...
        while self.macro_running:
            ...
            if fishing_mode == "color":
                ...
                colors = self.get_colors_at_lines(fish_img, line_coords)
                if is_initial_run == True:
                    fish_x = np.mean(line_coords)
                    line_mean = int(round(len(line_coords) / 2))
                    left_x = line_coords[line_mean - 1]
                    right_x = line_coords[line_mean + 1]
            ...
            if fish_overlay == "on":
                if fishing_mode == "color":
                    ...
                    self.fish_overlay.draw_box(
                        bar_center=bar_center, box_size=bar_size,
                        color="cyan", canvas_offset=0,
                        show_bar_center=True, bar_y1=0.2, bar_y2=0.8
                    )
                    for pos in range(len(line_coords)):
                        self.fish_overlay.draw_box(bar_center=line_coords[pos], box_size=0, color=colors[pos], canvas_offset=0)
